...
class TextSystem(object):
    ...
    def close(self):
        """释放 det/rec/cls 后端 session 资源。"""
        for component in (self.text_detector, self.text_recognizer, getattr(self, "text_classifier", None)):
            if component is None:
                continue
            for attr in ("det_onnx_session", "rec_onnx_session", "cls_onnx_session"):
                session = getattr(component, attr, None)
