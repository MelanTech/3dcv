...
class TextClassifier(PredictBase):
    def __init__(self, args):
        ...
        fixed_shape = self.fixed_input_shape(self.cls_onnx_session)
        if fixed_shape is not None:
            batch, channels, height, width = fixed_shape
            self.cls_image_shape = [channels, height, width]
            self.cls_batch_num = max(1, min(int(self.cls_batch_num), int(batch)))
