...
class TextRecognizer(PredictBase):
    def __init__(self, args):
        ...
        fixed_shape = self.fixed_input_shape(self.rec_onnx_session)
        if fixed_shape is not None:
            batch, channels, height, width = fixed_shape
            self.rec_image_shape = [channels, height, width]
            self.rec_batch_num = max(1, min(int(self.rec_batch_num), int(batch)))
    ...
    def resize_norm_img(self, img, max_wh_ratio):
        ...
        # 静态 .om 后端要求固定宽度，动态 ONNX 后端可按最大宽高比扩宽。
        if getattr(self.rec_onnx_session, "backend_name", "onnx") == "acl":
