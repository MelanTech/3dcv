...
class TextDetector(PredictBase):
    def __init__(self, args):
        ...
        # 初始化模型。ACL 的 .om 通常是静态输入，后续预处理会从模型 shape 反推尺寸。
        ...
        fixed_shape = self.fixed_input_shape(self.det_onnx_session)

        if fixed_shape is not None:
            _, _, input_h, input_w = fixed_shape
            resize_config = {
        ...
        pre_process_list = [
            {
                "DetResizeForTest": resize_config
            },
            {
                "NormalizeImage": {
